# pytest_ui_api_template

## Шаблон для автоматизации тетирования на python проекта "Кинопоиск"

### Проект онлайн-кинотеатр “Кинопоиск” предназначен для поиска и просмотра фильмов, сериалов, получения информации о кинематографе

### Шаги:
1. Склонировать проект 'git clone https://github.com/AngelLANister/pytest_ui_api_template.git'
2. Установить все зависимости
3. Запустить тесты 'pytest'
4. Сгенерировать отчет 'allure generate allure-files -o allure-report'
5. Открыть отчет 'allure open allure-report'

### Стек:
- pytest
- selenium
- webdriver manager
- requests
- allure
- config
- json

### Структура:
- ./test - тесты
- ./pages - описание страниц
- ./api - хеоперы для работы с api

### Полезные ссылки:
- [Подсазка по Markdown](https://www.markdownguide.org/cheat-sheet/)
- [Генератор файла .gitignore](https://www.toptal.com/developers/gitignore/)
- [финальный проект по ручному тестированию](https://qa1142skypro.yonote.ru/share/1997b178-b556-4739-aaa6-ad54793d5e40)

### Библтотеки
- pip install pytest
- pip install selenium
- pip install webdriver-manager
- pip install allure-pytest
- pip install requests